...
The models belong to the `PostModel` type, they have corresponding `train(m, X, Y)` methods for calibrating the model `m` to the provided inputs `X` and targets `Y`, and `predict(m, x)` methods yielding quantiles of the predictive distribution conditional on input `x`.
