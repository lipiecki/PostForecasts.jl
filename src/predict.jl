...
function predict(m::Union{QR{F}, LassoQR{F}, SQR{F}}, input::AbstractVector{<:Number})::Vector{F} where {F<:AbstractFloat}
    Base.require_one_based_indexing(input)
    nreg(m) == length(input) || throw(ArgumentError("model `m` requires $(nreg(m)) regressors, but $(length(input)) were provided"))
    _predict(m, input)
end

function predict(m::Union{QR{F}, LassoQR{F}, SQR{F}}, input::Number)::Vector{F} where {F<:AbstractFloat}
    nreg(m) == 1 || throw(ArgumentError("model `m` requires $(nreg(m)) regressors, but one was provided"))
    _predict(m, [input])
end
...
function predict!(m::Union{QR, LassoQR, SQR}, output::AbstractVector{<:AbstractFloat}, input::AbstractVector{<:Number})::Nothing
...
function predict!(m::Union{QR, LassoQR, SQR}, output::AbstractVector{<:AbstractFloat}, input::Number)::Nothing
...
predict!(m::Union{QR, LassoQR, SQR}, output::AbstractVector{<:AbstractFloat}, input::Number, ::AbstractVector{<:AbstractFloat}) = predict!(m, output, input)
predict!(m::Union{QR, LassoQR, SQR}, output::AbstractVector{<:AbstractFloat}, input::AbstractVector{<:Number}, ::AbstractVector{<:AbstractFloat}) = predict!(m, output, input)
