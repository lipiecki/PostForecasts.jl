...
### LEAR forecasts
Point forecasts were generated using a LASSO-Estimated AutoRegressive (LEAR) model [(Lago et al., 2021)](https://doi.org/10.1016/j.apenergy.2021.116983). The regressors used in the model include past prices, day-ahead predictions of the system-wide load, wind generation and solar generation. The parameters are estimated separately for each of the 4 training window lengths, i.e., 56, 84, 1092 and 1456 most recent days and employ cross-validation for selecting a regularization penalty.

### Loading forecasts
To load the forecasts from the EPEX dataset, you can use the `loadDataset` function. For example, to get the `ForecastSeries` corresponding to hour 00:00, run:
