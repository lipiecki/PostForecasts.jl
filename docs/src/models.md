...
The predictive distribution conditional on point forecast $\hat{y}$ is a Gaussian $\mathcal{N}(\mu + \hat{y}, \sigma)$, where $\mu$ and $\sigma$ are mean and sample standard deviation of errors in the training window.
